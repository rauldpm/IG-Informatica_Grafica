...
Escena::Escena(){
  ...
  if(opcion == 'S'){
    ...
    textura = new Textura(archivo);
     ...
     textura = new Textura("data/imagenes_texturas/logo.jpg");
  }
...
void Escena::teclaEspecial(int Tecla1,int x,int y) {
  switch (Tecla1){
  	...
  	case GLUT_KEY_PAGE_UP:   zoom++; break;
  	case GLUT_KEY_PAGE_DOWN: if(zoom > 1) zoom--; break;
...
void Escena::change_projection()  {
  ...
  glLoadIdentity();
  glFrustum(-Width/zoom*ratio,Width/zoom*ratio,-Height/zoom*ratio,Height/zoom*ratio,Front_plane,Back_plane);
}
